...
function animateCounters() {
    ...
    statBoxes.forEach(statBox => {
        const target = parseInt(statBox.textContent.replace('%', ''), 10);
        if (isNaN(target)) return; // Ignorar se o valor não for um número válido

        const increment = target / speed;

        const updateCount = () => {
            const currentCount = parseInt(statBox.textContent.replace('%', ''), 10) || 0;
            if (currentCount < target) {
                statBox.textContent = Math.ceil(currentCount + increment) + (statBox.textContent.includes('%') ? '%' : '');
                setTimeout(updateCount, 10); // Intervalo ajustado para evitar sobrecarga
            } else {
